...
- **Bonsai-driven (default)** queries the Bonsai MongoDB for every sample whose `pipeline.analysis_profile` contains the profile, the same query `check-backup` uses. It takes each sample's `sample_name` and `lims_id` from Bonsai. Its `groups` are the profile's species (`saureus`, `ecoli`, `mtuberculosis`, `spyogenes`, `streptococcus`, `staphylococcus`, `klebsiella`), the same value JASEN sets as `params.groups`, followed by any groups found by a reverse lookup of `sample_group.included_samples`.
- **Tree-driven (`--no-bonsai`)** skips Bonsai and finds samples by scanning the backup tree. Every filename under the profile's output dirs that ends in a declared `<mask><file_ext>` suffix contributes its prefix as a `sample_id`, taken as a union across all outputs, so a sample turns up as long as one of its files is there. Stripping a known suffix recovers the `sample_id` even when it contains underscores. Wildcard-mask outputs and `_versions.yml` files are ignored. Nothing connects to MongoDB, so `--db-name`/`--db-collection` aren't needed. Bonsai's metadata isn't available in this mode, so `sample_name` falls back to `sample_id` and `lims_id` is left unset. `groups` still gets the profile's species.

Either way, the analysis-result files are located with the same per-profile output declarations `check-backup` uses (`jasentool/config.py`), and a `software_name` → `create-yaml` field mapping (`CREATE_YAML_FIELD_MAP`, also in `jasentool/config.py`) says which flag each file feeds. Outputs with no matching `create-yaml` field are skipped, such as `mask_polymorph` and `format_jasen`. The `*_meta.json` database files are handled separately (see **Database versions** below). Alignment QC (`samtools_coverage`, `samtools_stats`, `samtools_bedcov`, and the legacy `post_align_qc` of JASEN 1.2.0 and earlier) is declared for every profile except the generic `streptococcus` one, which has no reference genome; when a sample has both, the samtools outputs win and the legacy file is dropped. The reference-aligned BAM and its index (`bwa_mem_ref`, `samtools_index_ref`) become the IGV alignment track. When a profile has more than one possible IGV `vcf` source (TB's `tbprofiler_vcf`/`snippy_vcf` versus non-TB's `freebayes`), the first match in `CREATE_YAML_VCF_PRIORITY` order wins.

**Run metadata (`nextflow_run_info` and `lims_id`).** JASEN's `save_analysis_metadata` step writes `<backup-dir>/<species>/analysis_metadata/<sample_id>_analysis_meta.json`. It holds the run info (`pipeline`, `version`, `commit`, `release_life_cycle`, `command`, `analysis_profile`, `sequencing_run`, and so on) as well as `lims_id` and `sample_name`. rebuild-manifests reads `lims_id` from it (and `sample_name` in `--no-bonsai` mode), then writes a copy into the output directory as `<sample_id>_analysis_meta.json` and points the manifest's `nextflow_run_info` at that copy. bonsai-prp requires all of these. When Bonsai has a `lims_id` or `sample_name`, that value wins and the metadata only fills the gap. If the file is missing, the sample is still written, but a warning notes that the manifest will lack `nextflow_run_info` and `lims_id`. This is the same file `format-cdm` parses, via `jasentool/cdm/loader.py`.
...
**Reference genome.** The manifest's `reference_genome_accession` is the **assembly** accession of the reference the sample was aligned against (e.g. `GCF_000012045.1` for *S. aureus*). It must match a reference genome registered in Bonsai, which stores the corresponding chromosome/plasmid accessions (e.g. `NC_002951.2`) itself and uses them to build IGV loci — so the manifest no longer needs to carry a contig accession. Supply it per run with `--reference-genome-accession`; with no flag the field is left off the manifest and the sample simply has no genome browser. `create-yaml` takes the same option.

**Database versions (`database_info`).** JASEN writes a `*_meta.json` next to its resfinder, virulencefinder and serotypefinder results holding the database versions it screened against; `CREATE_YAML_DATABASE_INFO` in `jasentool/config.py` lists them. They're passed to `create-yaml --database-info`, which writes `database_info` entries of `software`, `name` and `version` into the manifest. Where a meta file is missing (TB has none, so tbdb always comes from the fallback) or records `unknown`, the version that release pinned is used instead. `DATABASE_VERSIONS_FALLBACK` holds submodule commits for `1.0.0`-`1.1.2`, when the databases were git submodules, and Makefile versions from `1.2.0` on.
...
**Filling missing versions (`--jasen-version`).** Some tools have no usable version in the tree: either no `_versions.yml` was written (chewbbaca, for example), or the file records only a database version (`virulencefinder_db`) and not the tool version. Pass `--jasen-version <RELEASE>`, the JASEN release the samples were run with, to fill those gaps. `VERSIONS_FALLBACK` in `jasentool/config.py` maps each release from `1.0.0` to `1.3.0` to the tool versions of the containers that release pinned (abridged):

```python
VERSIONS_FALLBACK = {
    "1.1.0": {"amrfinderplus": "3.11.11", "chewbbaca": "3.3.2"},
    "1.3.0": {"amrfinderplus": "4.2.7", "chewbbaca": "3.5.3"},
}
```

The tree always wins: a fallback value is used only when a tool the sample has an output file for has no version from the tree. Keys are the software name as it appears *inside* a versions.yml (what `create-yaml` looks up), not the container image name, so `amrfinderplus` rather than `ncbi-amrfinderplus`, `bracken` for kraken/bracken, and `tb-profiler`. Only versions relevant to the sample's outputs are applied; other entries in the release's map are ignored. Each fill is logged (`<sample_id>: filled N version(s) from fallback: ...`). Without `--jasen-version`, nothing is filled. To support a new release, add its container versions to `VERSIONS_FALLBACK`.

**Files Bonsai reads from disk (`--symlink-dir`).** The BAM, BAI, VCF, sourmash signature and SKA index aren't uploaded to Bonsai; the manifest only records their paths and the API serves them from disk. JASEN symlinks these into `params.symlink_dir` (`/access/jasen` at CMD), which is what the API mounts, so pass `--symlink-dir` to have the manifest point there rather than into the backup tree. Only those five fields are affected; everything else still comes from `--backup-dir`. The symlink tree groups by file type rather than by tool, so they are looked up under `bam/`, `vcf/`, `ska/` and `sourmash/` — snippy's VCF lives in `vcf/`, not `snippy/`. A file that is in the backup tree but missing from the symlink tree is left out with a warning, because a backup path is one the API rejects with `Outside allowed directory`.

```
jasentool rebuild-manifests --profile <PROFILE> --backup-dir <DIR>
                            -o <OUTPUT_DIR>
                            (--db-name <DB> --db-collection <COLLECTION> | --no-bonsai)
                            [--symlink-dir <DIR>] [--address <URI>]
                            [--db-collection-groups <COLLECTION>]
                            [--sample-id <ID>] [--jasen-version <RELEASE>]
                            [--reference-genome-accession <ACC>]
```
...
| `--profile` | Yes | — | JASEN profile name, e.g. `staphylococcus_aureus` |
| `--backup-dir` | Yes | — | Root of the backup storage tree |
| `--symlink-dir` | No | — | Root of the JASEN symlink tree (`params.symlink_dir`). When set, the BAM, BAI, VCF, sourmash signature and SKA index are taken from here instead of the backup tree, so their paths are ones Bonsai can read |
| `-o`/`--output-dir` | Yes | — | Directory to write `<sample_id>_bonsai.yaml` and `<sample_id>_versions.yml` |
| `--no-bonsai` | No | False | Discover samples by scanning the backup tree instead of querying Bonsai. `sample_name` falls back to `sample_id` and `lims_id` is left unset; `groups` still gets the profile's species |
...
| `--jasen-version` | No | — | JASEN release (`1.0.0`–`1.3.0`) the samples were run with; fills a version the backup tree lacks for a tool from that release's containers (the tree always wins) |
| `--reference-genome-accession` | No | — | Assembly accession stamped on every manifest as `reference_genome_accession` |

**Outputs** (per sample, in `--output-dir`)

- **`<sample_id>_versions.yml`** — the merged per-process versions file, including any `--jasen-version` fallback fills. Omitted only when neither the tree nor the fallback produced a version for the sample.
- **`<sample_id>_analysis_meta.json`** — a copy of the run metadata with `release_life_cycle` normalized, referenced by the manifest's `nextflow_run_info`. Written only when the backup tree has a metadata file for the sample.
