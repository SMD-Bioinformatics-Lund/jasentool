...
- **`<stem>_assemblies.csv`** — *backed-up FASTA assemblies, filtered to `sample_name != sample_id`*. One row per existing `<sample_id>_<assembler>.fasta` in `<backup-dir>/<species>/fasta/`, where `<assembler>` is one of `spades`, `skesa`, `flye`, `medaka`. Columns: `sample_id, sample_name, lims_id, profile, assembler, assembly_path`. Useful as an input manifest when re-running an updated tool (e.g. chewbbaca) on the same assemblies. Samples flagged as `name_equals_id` in `<stem>_review.csv` are excluded.
